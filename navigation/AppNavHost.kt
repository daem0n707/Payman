...
fun AppNavHost() {
    ...
    var swiggyDineoutEnabled by remember { mutableStateOf(loadSwiggyDineoutOption(context)) }
    ...
    when (currentScreen) {
        ...
        "addBill" -> AddBillUI(
            apiKey = apiKey,
            onBillProcessedRequest = { bitmap, uri ->
                ...
                scope.launch {
                    processBillImage(context, bitmap, apiKey, uri, 
                        onSuccess = { realBill ->
        ...
        "billDetails" -> BillDetailsUI(
            ...
            swiggyDineoutOptionEnabled = swiggyDineoutEnabled
        )
        "people" -> PeopleUI(
            people = people,
            onDismiss = { currentScreen = "home" },
